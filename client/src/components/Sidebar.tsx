...
const Sidebar = ({isMenuOpen,  project, setProject, isGenerating, setIsGenerating}: 
  ...
  return (
    <div className={`h-full sm:max-w-sm  rounded-xl bg-gray-900 border-gray-800 transition-all 
    ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
        ...
        <div className='flex-1 overflow-y-auto no-scrollbar px-3 flex flex-col gap-4'>
                ...
                if(isMessage){
                    ...
                    return (
                        ...
                        isUser ? "justify-end" : "justify-start"}`}>
                            ...
                            <div className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${
                                isUser ? "bg-linear-to-r from-indigo-500 to-indigo-600 text-white rounded-tr-none"
                    ...
                    return(
                        <div key={ver.id} className='w-4/5 mx-auto my-2 p-3 rounded-xl
                         bg-gray-800 text-gray-100 shadow flex flex-col gap-2'>
                            ...
                            (
                                <button onClick={()=>handleRollback(ver.id)}
                                 className='px-3 py-1 rounded-md text-xs
                                 bg-indigo-500 hover:bg-indigo-600 text-white'>Roll back to this version</button>
                            )}
            ...
            {isGenerating && (
                <div className='flex items-start gap-3 justify-start'>
                    <div className='w-8 h-8 rounded-full bg-linear-to-br from-indigo-600
        ...
        <form onSubmit={handleRevisions}  action="" className='m-3 relative'>
                <div className='flex items-center gap-2'>
                    ...
                    <button disabled={isGenerating || !input.trim()} className='absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600
