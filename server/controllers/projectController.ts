...
const userId = req.userId;
    ...
    if(user.credits < 5){
        return res.status(403).json({message: "Insufficient credits. Add more to make changes"});
    }

    if(!message || message.trim() === ''){
        return res.status(400).json({message: "Please provide a valid message for revision"});
    }
...
Return ONLY the enhanced request, nothing else. Keep it concise (1-2 sentences).
    ...
    Apply the requested changes while maintaining the Tailwind CSS styling approach.`
                ...
                role: "user",
                content: `Here is the current code of the website: "${currentProject.current_code}" 
    ...
    res.json({credits: 'Changes made successfully'});
}catch(error: any){
    await prisma.user.update({
        where: {id: userId},
        data: {credits:{increment: 5}}
