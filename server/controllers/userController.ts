        ...
        messages:[
            {
                ...
                website request and expand it into a detailed, comprehensive prompt that will 
                help create the best possible website.
...
Return ONLY the enhanced prompt, nothing else.
 Make it detailed but concise (2-3 paragraphs max).`
    ...
    const codeOutput = codeGenerationResponse.choices[0].message.content || '';
    if(!codeOutput){
        await prisma.conversation.create({
        data:{
            role: 'assistant',
            content:"Unable to generate code, please try again later.",
            projectId: project.id
        }
    })
    await prisma.user.update({
    ...
    await prisma.conversation.create({
        data:{
            role: 'assistant',
            content: "Website generation completed! You can preview it and request any changes.",
            projectId: project.id
        }
    })
    ...
    await prisma.websiteProject.update({
        where: {id: project.id},
        data:{
            current_code: codeOutput.replace(/```[a-z]*\n?/gi, '')
            .replace(/```$/g, '')
            .trim(),
